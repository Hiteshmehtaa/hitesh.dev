...
autopilot_old = """      if (autopilotTarget) {
...
autopilot_new = """      // Break out of autopilot if manual controls are used
...
testAuto_old = """      } else if (testAutoDrive) {
...
testAuto_new = """      } else if (testAutoDrive) {
        const dx = 0 - chassisBody.position.x;
        const dz = -35 - chassisBody.position.z;
        const dist = Math.sqrt(dx*dx + dz*dz);
        const speed = chassisBody.velocity.length();
...
cam_old = """         // Upright Chase Camera (No roll/pitch orientation tracking to avoid disorientation)
